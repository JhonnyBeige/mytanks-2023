...
public class RemoteDatabaseLogger {
   ...
   public static String dumpLogs() {
      ...
      List<RemoteDatabaseLogger.DumpData> dump = new ArrayList();
      Iterator var4 = logs.iterator();

      while(true) {
         while(true) {
            while(var4.hasNext()) {
               LogObject log = (LogObject)var4.next();
               ...
               if (dump.contains(dd)) {
                  ...
                  while(var7.hasNext()) {
                     RemoteDatabaseLogger.DumpData _dd = (RemoteDatabaseLogger.DumpData)var7.next();
                     if (_dd.equals(dd)) {
                        ++_dd.count;
                        break;
                     }
            ...
            while(var4.hasNext()) {
               RemoteDatabaseLogger.DumpData _dd = (RemoteDatabaseLogger.DumpData)var4.next();
   ...
   public static List<RemoteDatabaseLogger.DumpData> getDump() {
      List<LogObject> logs = databaseManager.collectLogs();
      List<RemoteDatabaseLogger.DumpData> dump = new ArrayList();
